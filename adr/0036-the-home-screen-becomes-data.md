...
[#253](https://github.com/correctiv/correctiv-app/pull/253), the election night first; §13's sample data holds the pinned articles still and not the feeds, and the preview says so. §15 is built in
...
~~`?scenario=<name>`~~ `sc=<name>` (spelled so when it was built, in #253 on 2026-09-23, two letters like the preview's other later keys; no record changed it), beside the device, route and appearance ADR 0028 already puts there. A
