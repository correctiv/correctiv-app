...
Sixty-two records shaped this repo. Read them when you want to know *why* something
...
| [0062](0062-the-texts-submission-may-change-wordings-and-nothing-else.md) | The texts submission may change wordings, and nothing else | accepted, **built the same day**, not run against github.com; six decisions, carrying out 0061 §3's `[texte]` kind with the dev server's own validator, write and German, replacing the one-file assertion with an allow-list of catalogue files and a proof that reads each changed file with the scanner against `main` and lets only the submitted literals differ, refusing a wording over a thousand characters or with a character a reviewer cannot see, refusing a submission whole for one bad entry and any issue with an HTML comment in it, and sending the wordings one per line after measuring that 27 realistic edits still fit the address; strikes one clause and one open item of 0061 and one open item of 0056, three open items named |
