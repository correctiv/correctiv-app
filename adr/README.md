...
| [0064](0064-ci-cd-for-a-public-repository.md) | CI/CD for a public repository | draft, **not built**; seven decisions: GitHub Actions and Fastlane rather than EAS because the repository is public and Actions costs it nothing, which is written down as the condition; pull requests build both platforms without a secret; the review app is a second identity with its own name, a banded icon and its own scheme so a tester keeps both apps on one device; the build number is the run number and only the newest `main` is delivered; the review APK is the asset of a fixed `preview` prerelease that the site links to, keeping Pages out of it; signing stays with CORRECTIV through a read-only Match repository; no ad-hoc iOS builds. Retires nothing until built, then one consequence of 0011 |
