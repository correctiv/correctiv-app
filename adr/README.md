...
| [0004](0004-react-native-pivot.md) | Move to React Native / Expo, with a web target | accepted; two sentences on the web reader's sandbox retired by 0065 |
...
| [0065](0065-embeds-inline-from-a-short-list-and-a-link-for-the-rest.md) | Embeds render inline from a short list, and the rest become a link | accepted, **built the same day**; six decisions after measuring what each embed in 300 posts loads: Datawrapper, CORRECTIV's own apps, 23degrees, Flourish and DocumentCloud render, the last as the borderline case, and the list is a proposal the newsroom may change in one place; YouTube, LinkedIn and Instagram become a link to the browser, a correctiv.org post a link into the reader; the document forbids itself script by policy, which lets the web frame carry `allow-scripts` for the embeds; strikes two sentences in 0004 and one in 0017, iOS and Android unrun |
