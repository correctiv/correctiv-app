...
| [0052](0052-the-sites-own-words-follow-the-setting.md) | The site's own words follow the setting, the repository's are printed as they are written | accepted; four decisions, replaces a line drawn by position with one drawn by author after measuring a page whose filter was English inside a German shell, holds it with the app's own language-blind literal check over a per-file ratchet, shares that walk through prose-and-code while each check keeps its own argument, and leaves `content/` quoted rather than narrated with the open editorial questions named as the case against; strikes the line in 0050 §2 and retires one sentence of AGENTS.md |
