...
| [0049](0049-the-catalogue-is-a-package.md) | The catalogue is a package, and the locale comes from the host | accepted, **carried out**: §1, §2, §5 and §6 in #219, §3 and §4 in #221; six decisions, takes the strings out of the app so the core's own German stops living in one of its hosts, names four things that deliberately do not move with them, strikes one claim in 0026 §6 that was wrong when written and leaves a second for the record that voids it, retires one sentence of AGENTS.md, the one-language checks split between the package and the app |
