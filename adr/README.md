...
Sixty-four records shaped this repo. Read them when you want to know *why* something
...
| [0065](0065-embeds-inline-from-a-short-list-and-a-link-for-the-rest.md) | Embeds render inline from a short list, and the rest become a link | accepted, **built the same day**; seven decisions after measuring what each embed in 300 posts loads: Datawrapper, CORRECTIV's own apps, 23degrees, Flourish and DocumentCloud render, the last as the borderline case, and the list is a proposal the newsroom may change in one place; YouTube, LinkedIn and Instagram become a link to the browser, a correctiv.org post a link into the reader; the document forbids itself script by policy, which lets the web frame carry `allow-scripts` for the embeds; §4, §5 and §7 revised after a cold review found a `<meta>` refresh turning the web reader into a page on the app's own origin, so the document builder gates every body for what acts without a script and Datawrapper and Flourish frames get no origin of their own; strikes a sentence and a half in 0004 and one in 0017, iOS and Android unrun |
